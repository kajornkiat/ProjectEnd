...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Future<void> fetchImages() async {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        profileImageUrl = 'http://10.39.5.52:3000${data['profile_image']}';
        backgroundImageUrl =
            'http://10.39.5.52:3000${data['background_image']}';
  ...
  Future<void> uploadImage(File imageFile, String type) async {
    ...
    final request = http.MultipartRequest(
      'POST',
      Uri.parse('http://10.39.5.52:3000/profile'),
    );
  ...
  Future<void> updateProfile({
    ...
    final request = http.MultipartRequest(
      'POST',
      Uri.parse('http://10.39.5.52:3000/profile'), // ใช้ URL ที่ถูกต้อง
    );
