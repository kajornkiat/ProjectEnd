...
class _FeedsviewsPageState extends State<FeedsviewsPage> {
  ...
  Future<void> fetchPosts({String? searchQuery}) async {
    String url = 'http://10.39.5.52:3000/api/posts';
  ...
  Future<void> submitPost() async {
    ...
    var request = http.MultipartRequest(
      'POST',
      Uri.parse('http://10.39.5.52:3000/api/posts'),
    );
  ...
  Future<void> deletePost(int postId) async {
    ...
    try {
      final response = await http.delete(
        Uri.parse('http://10.39.5.52:3000/api/posts/$postId'),
  ...
  Future<void> fetchComments(int postId) async {
    try {
      final response = await http.get(
        Uri.parse('http://10.39.5.52:3000/api/comments/$postId'),
      );
  ...
  Future<void> addComment(int postId, Function updateState) async {
    ...
    final response = await http.post(
      Uri.parse("http://10.39.5.52:3000/api/comments"),
      headers: {
  ...
  Future<void> deleteComment(
    ...
    final response = await http.delete(
      Uri.parse("http://10.39.5.52:3000/api/comments/$commentId"),
      headers: {
        'Authorization': 'Bearer $token',
      },
    );
                          ...
                          return ListTile(
                            leading: CircleAvatar(
                              backgroundImage: NetworkImage(
                                'http://10.39.5.52:3000${comment['profile_image']}',
                              ),
                            ),
                  ...
                  children: [
                    CircleAvatar(
                      backgroundImage: widget.userData['profile_image'] != null
                          ? NetworkImage(
                              'http://10.39.5.52:3000${widget.userData['profile_image']}')
                      ...
                      return Card(
                        ...
                        child: Column(
                          ...
                          children: [
                            ListTile(
                              leading: CircleAvatar(
                                backgroundImage: post['profile_image'] != null
                                    ? NetworkImage(
                                        'http://10.39.5.52:3000${post['profile_image']}')
                                    : AssetImage(
                            ...
                            if (post['image'] != null)
                              Image.network(
                                  'http://10.39.5.52:3000/posts/${post['image']}'),
                            Padding(
                              padding: EdgeInsets.all(8.0),
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
